    ...
    def eval(self):
        ...
        if comm.is_main_process() and self.trainer.cfg.get("use_mlflow", True):
            ...
            if ml_logger is not None:
                # 1. Main Metrics
                ml_logger.log_metric("Val_Loss", loss_avg, current_epoch)
                ml_logger.log_metric("Val_mIoU", m_iou, current_epoch)
                ml_logger.log_metric("Val_mAcc", m_acc, current_epoch)
                ml_logger.log_metric("Val_allAcc", all_acc, current_epoch)
                
                # 2. Class-by-Class Metrics
                for i in range(self.trainer.cfg.data.num_classes):
                    class_name = self.trainer.cfg.data.names[i]
                    # We format the name exactly how you requested: "Class_0-Unclassified IoU"
                    ml_logger.log_metric(f"Class_{i}-{class_name}_IoU", iou_class[i], current_epoch)
                    ml_logger.log_metric(f"Class_{i}-{class_name}_Acc", acc_class[i], current_epoch)
        # ==========================================================
...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        current_epoch = self.trainer.epoch + 1
        # ==========================================================
        # MLFLOW: LOG ALL VALIDATION METRICS
        # ==========================================================
        if comm.is_main_process() and self.trainer.cfg.get("use_mlflow", True):
            ml_logger = get_mlflow_logger()
            if ml_logger is not None:
