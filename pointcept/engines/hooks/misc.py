...
from pointcept.mlflow_tools.mlflow_logger import get_mlflow_logger
...
class InformationWriter(HookBase):
    ...
    def after_epoch(self):
        ...
        if comm.is_main_process() and self.trainer.cfg.get("use_mlflow", True):
            ml_logger = get_mlflow_logger()
            if ml_logger is not None:
                for key in self.model_output_keys:
                    # key is usually 'loss'
                    ml_logger.log_metric(f"Train_{key}", self.trainer.storage.history(key).avg, self.trainer.epoch + 1)
        if self.trainer.writer is not None:
            for key in self.model_output_keys:
                self.trainer.writer.add_scalar(
                    "train/" + key,
...
class CheckpointSaver(HookBase):
    ...
    def after_epoch(self):
        if is_main_process():
            ...
            # (Keeps your graph drawing a continuous line)
            ml_logger = get_mlflow_logger()
            if ml_logger is not None and self.trainer.cfg.evaluate:
                ml_logger.log_metric(f"Best_{current_metric_name}", self.trainer.best_metric_value, self.trainer.epoch + 1)
            ...
            # ==========================================
            # MLFLOW: OVERWRITE RAW .PTH ARTIFACT ONLY
            # ==========================================
            if is_best:
                best_model_path = os.path.join(self.trainer.cfg.save_path, "model", "model_best.pth")
                shutil.copyfile(
                    filename,
                    best_model_path,
                )
                
                if ml_logger is not None and self.trainer.cfg.get("use_mlflow", True):
                    # Only upload the raw weights to Artifacts. NO Registry here!
                    ml_logger.log_artifact(best_model_path)
                    print("[MLflow] New high score! Overwrote model_best.pth in Artifacts.")
    ...
    # ==========================================
    # MLFLOW: FINAL REGISTRY UPLOAD
    # ==========================================
    def after_train(self):
        if is_main_process() and self.trainer.cfg.get("use_mlflow", True):
            ml_logger = get_mlflow_logger()
            if ml_logger is not None:
                self.trainer.logger.info("[MLflow] Training complete! Packaging Best Model for the Registry...")
                
                best_model_path = os.path.join(self.trainer.cfg.save_path, "model", "model_best.pth")
                
                if os.path.exists(best_model_path):
                    # 1. Load the TRUE BEST weights back into the model brain
                    checkpoint = torch.load(best_model_path, map_location="cpu")
                    
                    # Pointcept adds 'module.' prefix for multi-GPU. We need to handle that gracefully.
                    weight = checkpoint["state_dict"]
                    if comm.get_world_size() == 1:
                        weight = {k.replace("module.", ""): v for k, v in weight.items()}
                        
                    self.trainer.model.load_state_dict(weight, strict=False)
                    
                    # 2. Package and Register it exactly ONE time
                    ml_logger.log_pytorch_model(self.trainer.model, "Pointcept_Best_Model")
                    self.trainer.logger.info("[MLflow] Successfully pushed the final Best Model to the Registry!")
